...
export default class Slider extends React.Component<IProps, IState> {
  ...
  render() {
    const { trackColor, activeColor, fontColor, style, isBright } = this.props;
...
class Percent extends React.Component<IPercentProps, IPercentProps> {
  ...
  render() {
    const { percent, height, width, brightWidth, colorOver, colorInner, isBright } = this.state;
      ...
      <View style={[styles.percent, { height, width }]}>
        <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}>
          {isBright ? (
            ...
            <Image style={[styles.percentIcon, { width: 32, height: 32 }]} source={images.ss} />
          )}
          <TYText style={[styles.percentText, { color: colorOver }]}>{percent}%</TYText>
        </View>
        <View
          style={{
            position: 'absolute',
        ...
        >
          {isBright ? (
            ...
            <Image style={[styles.percentIcon, { width: 32, height: 32 }]} source={images.s} />
          )}

          <TYText style={[styles.percentText, { color: colorInner }]}>{percent}%</TYText>
        </View>
      </View>
    );
  }
